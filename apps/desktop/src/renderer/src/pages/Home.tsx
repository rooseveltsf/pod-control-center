      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1440px]">
        ...
        >
            ...
            <div className="mt-2 flex flex-wrap items-end justify-between gap-5">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-400 sm:text-base">
                  Seu estúdio está pronto para mais uma produção.
                </p>
              </div>
              <Link
                to={paths.newAppointment}
                className="rounded-xl bg-violet-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-950/50 transition hover:bg-violet-400"
              >
                + Novo agendamento
              </Link>
            </div>
          ...
          <section className="mt-9 grid gap-4 sm:grid-cols-3">
            {[
              ["3", "pods configurados", "bg-violet-400"],
              ["1", "transmissão ao vivo", "bg-emerald-400"],
              ["2", "sessões hoje", "bg-amber-300"],
          ...
          <section id="pods" className="mt-10">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-white">Seus pods</h2>
            ...
            <div className="mt-4 grid gap-3 xl:grid-cols-3">
              {pods.map((pod) => (
                <article
                  key={pod.name}
                  className="group rounded-2xl border border-white/8 bg-white/[0.025] p-5 transition hover:-translate-y-0.5 hover:border-white/15 hover:bg-white/[0.045]"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-2.5 w-2.5 rounded-full ${pod.accent} ${pod.status === "Ao vivo" ? "animate-pulse" : ""}`}
                      />
                      <h3 className="font-semibold text-white">{pod.name}</h3>
                    </div>
                    <button
                      aria-label={`Opções do ${pod.name}`}
                      className="text-slate-500 hover:text-white"
                    >
                      •••
                    </button>
                  </div>
                  <p className="mt-6 text-sm text-slate-400">{pod.detail}</p>
                  <span
                    className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${statusStyle[pod.status]}`}
                  >
                    {pod.status}
                  </span>
                </article>
          ...
          <section
            id="agenda"
            className="mb-4 mt-10 rounded-2xl border border-white/8 bg-gradient-to-r from-white/[0.04] to-violet-500/[0.06] p-5 sm:flex sm:items-center sm:justify-between"
          >
            <div>
              ...
              <h2 className="mt-2 text-lg font-semibold text-white">
                Podcast Mesa 12 · 14:00
              </h2>
              <p className="mt-1 text-sm text-slate-400">
                Pod 02 · 2 participantes confirmados
              </p>
            </div>
            <button className="mt-4 rounded-xl border border-white/12 bg-white/[0.05] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/[0.1] sm:mt-0">
              Abrir sessão
            </button>
          </section>
        </main>
